...
void csTime(char *psz, bool f = true) {
  ...
  if (cs == 60) { cs = 0; }
  if (hA4 == h) { cm = mA4 - m - 1; }
  else if (hA4 > h) { temph = (hA4 - h) * 60; cm = (mA4 - m - 1) + temph; }
  else if (hA4 < h) { temph = ((hA4 - h) + 24) * 60; cm = (mA4 - m - 1) + temph; }
...
//  int temp_adc_val;
//  float temp_val;
//  temp_adc_val = analogRead(pin);
//  temp_val = temp_adc_val * (4.0 / 1024.0) * 100;
...
//            else if (buttonSet_read == LOW || c == 'm' ){
      ...
      case 2:   // Alarm 1
        alarm("072211940000012");
        break;

      case 3:   // Alarm 2
        alarm("Dimas Aditya Maulana Fajri");
        break;

      case 4:   // Alarm 3
        alarm("07211940000012-Dimas Aditya Maulana Fajri");
        break;
...
void alarm_menus(const char *Alarm, int hA, int mA, boolean alarmSet, int romhA, int rommA, int romsetA, bool f = true){
   if (!change_alarm_menu){
      ...
      if (buttonSel_read == LOW){
        ...
        if (setup_menu == 6) { setup_menu = 0; display = 0; }
        P.setTextEffect(0, PA_PRINT, PA_SCROLL_UP);
      }
...
void readKeyboardA5(){
  alarm5c[indexC] = c;
//  if(c == 'k') {alarm5c[indexC] = 0; c = 0;  change_alarm_menu = false;}
//  else if(c == 'b') {alarm5c[indexC] = 0; c = 0;  alarm_menu = alarm_menu - 1;}
//  else if(c == 'x') {alarm5c[indexC] = 0; c = 0;  display = 0;}
//  else if(c == 'd') {alarm5c[indexC] = 0; indexC = indexC - 1; c = 0;}
